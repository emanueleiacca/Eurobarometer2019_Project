...
head(dati %>% select(t, anno, quarter))
```
...
dati_corr <- dati %>% select(all_of(vars)) %>% na.omit()
...
```{r}
dati <- dati %>%
  select(-Spread_bp_fino1, -Spread_bp_oltre1 )
```
...
```{r}
dati <- dati %>%
  select(-t_operazioni_rifin_marg, -t_interesse_operazioni_rif_prin, -t_depositi_overnight)
...
```{r}
dati <- dati %>%
  select(-spread_lp_fino1, -spread_lp_oltre1)
# nel dubbio leviamo
```
...
dati_corr <- dati_filtrato %>% select(all_of(vars)) %>% na.omit()
cor_matrix <- cor(dati_corr)
...
dati_filtrato <- dati_filtrato %>%
  select(-tassi_interesse_bp_fino1)
```
...
Possiamos altare il Box-Cox, dato che già il modello è buono
```{r}
